...
void outputlayer::host_forward (matrix *neu0_ac, matrix *neu1_ac)
{
    ...
    //cout << " --- Fem el host_forward --- " << endl;
    //cout << " --- " << lognorm << "  " << smoothvalue << " --- " << endl;
    //lognorm = 9.044676;
    if (lognorm>0 && smoothvalue==1.0)    // only applied when no smoothing applied
    {
      //cout << "----------  Estem entrant  -------------" << endl;
        float sum = 0.0;
        for (int i=0; i<nrows; i++)
        {
            sum += srcac[i]*wgts[i+curword*nrows];
        }
        dstac[curword] = exp(sum-lognorm);
    }
    else
    {
      //cout << "----------  No entrem  -------------" << endl;
...
inputlayer::inputlayer (int nr, int nc, int mbsize, int cksize, int dim): layer(nr, nc, mbsize, cksize)
{
    ...
    else
    {
        ...
        ac_fea = new matrix (dim_fea, minibatch);
        ac_fea->initmatrix();
    }
}
