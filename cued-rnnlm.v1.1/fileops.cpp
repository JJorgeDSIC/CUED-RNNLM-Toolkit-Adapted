  ...
  while (!feof(fptr))
      ...
      if (c == '\n')
	{
	  ...
	  if (_word.size())
	    {
	      _word.push_back('\0');
	      linevec.push_back(_word.data());
	      _word.pop_back();
	      cnt ++;
	    }
	  break;
	}
      else if ((c == ' ' || c == '\t') && _word.size() == 0) // space in the front of line                                                         
	{
	  continue;
	}
      else if ((c == ' ' || c=='\t') && _word.size()) // space in the middle of line
	{
	  _word.push_back('\0');
	  if (cnt==0 && _word[0] != '<')
	    {
      ...
      else _word.push_back(c);
    }
  if (cnt>0 && (*(linevec.end()-1))[0] != '<' )
    {
      linevec.push_back("</s>");
      cnt ++;
    }
