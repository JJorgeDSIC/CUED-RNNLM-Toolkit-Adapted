...
void printusage(char *str)
{
    ...
    printf ("%35s\t%s\n", "-succeedwords <int>      :",        "specify the number of succeeding word for RNNLMs (default: 0)");
    printf ("%35s\t%s\n", "-succmergelayer <int>    :",        "specify the layer where previous and succ information merges (default: last layer)");
...
void parseArray2ints (string str, vector<int> &layersizes)
{
    int pos;
    int i, j;
    char numstr, typestr;
   ...
   while (str.size() > 0)
   {
       ...
       layersizes.push_back(string2int(substr));
       str = str.substr (pos+1);
   }
   layersizes.push_back(string2int(str));
}
