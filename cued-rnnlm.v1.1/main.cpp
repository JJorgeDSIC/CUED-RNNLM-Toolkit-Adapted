...
int main (int argc, char **argv)
{
    ...
    int device, minibatch, chunksize, k, independent,
            ...
            nthread, nclass, ptrnepoch, succwindowlength, succmergelayer;
    vector<string> layersizes;
    vector<int> sulayersizes;
	...
	if (!arg.empty())
    {
        ...
        str = arg.find("-succeedwords");
        if (!isEmpty(str))                      succwindowlength = string2int(str);
        else                                    succwindowlength = 0;
        assert (succwindowlength >=0);

        str = arg.find("-layers");
        if (!isEmpty(str))                      parseArray(str, layersizes);
        // else                                    return ARGSPARSEERROR;

        // which layer to merge the previous and future information
        str = arg.find ("-succmergelayer");
        if (!isEmpty(str))                      succmergelayer = string2int (str);
        else                                    succmergelayer = layersizes.size()-1;

        // layer structure for succeeding words
        // these layers are required to be feedforward with sigmoid in this code
        str = arg.find ("-succlayers");
        if (!isEmpty(str))                      parseArray2ints(str, sulayersizes);
        ...
        if (!isEmpty(arg.find ("-train")))
        {
            RNNLM rnnlm (inmodelname, outmodelname, inputwlist, outputwlist, layersizes, device, minibatch, chunksize, rand_seed, binformat);

            if (isEmpty(inmodelname))
            {
                rnnlm.setSuccVars (succwindowlength, sulayersizes, succmergelayer);
            }
            rnnlm.allocSuccwords ();
            rnnlm.setLRtunemode (lrtune);
            rnnlm.setTrainCritmode (traincrit);
            rnnlm.setNumncesample (k);
            rnnlm.setVRpenalty (vrpenalty);
            rnnlm.setIndependentmode (independent);
            rnnlm.setMinimprovement (min_improvement);
            rnnlm.setNthread (nthread);
            rnnlm.setMomentum (momentum);
            // set the default value for lognormconst in train is 0.0
            // if (lognormconst < 0)   lognormconst = 0.0;
            if (lognormconst > -10)
            {
            ...
            if (diaginit > 0)
            {
                rnnlm.setDiagInit (diaginit);
            }
            rnnlm.setDropoutRate (dropoutrate);
            rnnlm.setClipping (gradient_cutoff);
            rnnlm.setL2Regularization (l2reg);

            rnnlm.train(trainfile, validfile, learnrate, device, cachesize, fullvocsize, independent, debug);
            if (debug>0)
            {
                printf ("Finished RNNLM training\n");
            }
        }
        else if (!isEmpty(arg.find ("-ppl")))
        {
        ...
        else if (!isEmpty(arg.find("-sample")))
        {
            RNNLM rnnlm (inmodelname, inputwlist, outputwlist, fullvocsize, binformat, debug, minibatch, chunksize, device);
            rnnlm.setRandseed (rand_seed);

            rnnlm.sample(sampletextfile, unigramfile, nsample);
        }
        else
        {
            printusage(argv[0]);
