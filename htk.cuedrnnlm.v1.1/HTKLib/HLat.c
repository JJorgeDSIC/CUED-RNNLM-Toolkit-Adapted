...
static Boolean intpltLatGram = FALSE; /* interpolation between LM and lattice LM */
static Boolean logintLatGram = FALSE; /* log-linear interpolation between LM and lattice LM */
static float wgtLatGram = 0.5;        /* interpolation weight of lattice LM */
...
struct _SubLNode {
   ...
   LMState lmstate_mix[MAX_LMODEL];
   SubLArc *foll;
   SubLNode *next;
   LabId    fwordname[NSIZE];
   LogFloat score;
};
...
void InitLat(void)
{
   ...
   if (nParm>0){
      ...
      if (GetConfBool(cParm,nParm,"COMPRESSMERGE",&b))
         compressMerge = b;
      if (GetConfBool(cParm,nParm,"INTPLTLATGRAM",&b))
         intpltLatGram = b;
      if (GetConfBool(cParm,nParm,"LOGINTLATGRAM",&b))
         logintLatGram = b;
      if (GetConfFlt(cParm,nParm,"WGTLATGRAM",&f))
         wgtLatGram = f;
...
#if 0
      printf ("\n(xie0: %s %.8f) ", ln->word->wordName->name, exp(la->lmlike));
#endif
...
#define NBIN 1000
   ...
   if (trace & T_PRUN) {
      fprintf (stdout, "lattice pruned from %d/%d to %d/%d\n", lat->nn, lat->na, nn, na);
      fflush(stdout);
   }
...
*/
static LogFloat  LatLMTrans (LModel *lm, LMState src, LabId wordId, LMState *dest, LMState *src_mix, LMState *dest_mix, LabId *succwordId, LogFloat lmlike)
{
   LogFloat prob;
   ...
   if (wordId == nullWord) {
      ...
      wordId = startLMWord;
      prob = LMTrans (lm, src, wordId, dest, src_mix, dest_mix, succwordId, lmlike);
      return 0.0;
   }
...
*/
static SubLNode *FindAddSubLNode (MemHeap *heap, LNode *ln, LMState lmstate, LMState *lmstate_mix, int *nsln, LogFloat score, int ngramaprox)
{
   SubLNode *subln;
   int i;
   ...
   {
      /* checking both ngram LM state AND weight set state */
      NEntry *origne = subln->data.lmstate;
      NEntry *ne     = lmstate;
      ...
      if (ne)
      {
          ...
          if (i == ngramaprox
              && ne->fword[0] == origne->fword[0]
              && ne->fword[1] == origne->fword[1]
              && ne->fword[2] == origne->fword[2]
              )
          {
              if (score > subln->score)
              {
                  subln->score = score;
                  subln->data.lmstate = lmstate;
                  memcpy(subln->lmstate_mix, lmstate_mix, MAX_LMODEL*sizeof(LMState));
              }
              return subln;
          }
      }
...
{
   int i, nsln, nsla;
   LNode *ln, *newln, *fln, *ssln;
   LNode **topOrder;
   LArc *la, *newla, *fla, *ffla;
...
#if 0
   for (i = 0, ln = newlat->lnodes; i < newlat->nn; ++i, ++ln) {
      if (!ln->pred) {
         if (ln->word != newlat->voc->nullWord && ln->word->wordName != startLMWord) {
            fprintf(stdout, "ERROR: node I=%d t=%.2f W=%s has no inbound arcs !!!\n",
                    i, newlat->lnodes[i].time,
                    newlat->lnodes[i].word->wordName->name);
         }
      }
      if (!ln->foll) {
         if (ln->word != newlat->voc->nullWord && ln->word->wordName != endLMWord) {
            fprintf(stdout, "ERROR: node I=%d t=%.2f W=%s has no outbound arcs !!!\n",
                    i, newlat->lnodes[i].time,
                    newlat->lnodes[i].word->wordName->name);
         }
      }
...
#esle
      revlab[lat->nn - 3 - i]->aux = (Ptr) (GetWord(lat->voc, revlab[lat->nn - 3 - i], FALSE))->wordName->aux;
#endif
   }
...
#else
         fprintf(stdout, "%s ", (GetWord(lat->voc, revlab[j], FALSE))->wordName->name);
#endif
      }
      fprintf(stdout, "\n\n");
      fflush(stdout);
   ...
   for (i = 1, la = lat->larcs + 1; i < lat->na - 1; i++, la++) {
       ...
       if (i == lat->na-2)
       {
           succwordName = GetLabId("</s>", FALSE);
       }
       else
       {
           succwordName = (la+1)->end->word->wordName;
           if (strcmp(succwordName->name, "</s>")==0)
           {
               succwordName = GetLabId("<s>", FALSE);
           }
       }
      la->lmlike = GetLMProb(lm, &revlab[lat->nn - 2 - i], la->end->word->wordName, succwordName);

      if (lm->ppinfo) {
         lm->ppinfo->nword++;
         if (la->lmlike > 0.5 * EMINARG) {
            lm->ppinfo->loglike += la->lmlike;
            if (lm->ppinfo->oStreamFP) {
               fprintf(lm->ppinfo->oStreamFP, "%e\n", UNLOG_NATURAL(la->lmlike));
            }
         }
         else {
            lm->ppinfo->noov++;
      ...
      if (trace & T_EXP) {
         ...
         fprintf(stdout, ") -> %s : %e | %e\n", la->end->word->wordName->name, la->lmlike, UNLOG_NATURAL(la->lmlike));
         fflush(stdout);
      }
   }
