...
Lattice *NewILattice(MemHeap *heap,int nn,int na,Lattice *info)
{
   ...
   if (na==-1) {
      ...
      size_t na_new = (size_t)info->na*i;
      lat->larcs=(LArc *) New(heap, na_new);
      lat->na=info->na;
   }
   else if (na>0) {
      /* lat->larcs=(LArc *) New(heap, i*na); */
      size_t na_new = (size_t)na*i;
      lat->larcs=(LArc *) New(heap, na_new);
      lat->na=na;
   }
   else {
...
ReturnStatus WriteOneLattice(Lattice *lat,FILE *file,LatFormat format)
{
   ...
   for (i=0;i<lat->na;i++) {
      ...
      if (!(lat->format&HLAT_SHARC) && (format&HLAT_PRLIKE))
         OutputFloatField('r', ConvLogLikeToBase(lat->logbase, la->prlike), format&HLAT_LBIN, "%-7.3f", file);
